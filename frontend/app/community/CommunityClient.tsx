...
export function CommunityClient({ initialPosts }: { initialPosts: Post[] }) {
 ...
 <span className="text-muted-foreground text-[13px] font-medium">{formatDate(post.createdAt)}</span>
 ...
 <div key={index} className="rounded-2xl overflow-hidden border border-border bg-secondary max-h-[400px]">
 <img src={url.startsWith('http') ? url : `http://127.0.0.1:3000${url}`} alt={`Post attachment ${index + 1}`} className="w-full h-full object-cover"
 />
 </div>
 ))}
 </div>
 ...
 <button onClick={(e) => { e.stopPropagation(); handleOpenComments(post.id); }}
 ...
 <Button variant="ghost" size="icon" onClick={() => setIsCreatingPost(false)} className="rounded-full text-muted-foreground hover:bg-secondary">
 <X className="h-5 w-5" />
 </Button>
 </div>
 <div className="p-4">
 ...
 <div key={index} className="relative shrink-0 h-24 w-24 rounded-2xl overflow-hidden border border-border">
 ...
 <Button variant="ghost" size="icon" onClick={() => setActivePostForComments(null)} className="rounded-full text-muted-foreground hover:bg-secondary">
 <X className="h-5 w-5" />
 </Button>
 </div>
 ...
 <p className="text-[14px] text-muted-foreground whitespace-pre-wrap">{c.content}</p>
 </div>
 </div>
 ))
 )}
