  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-[24px] md:text-[28px] font-semibold text-foreground tracking-tight">{title}</h2>
        {viewMoreHref && (
      ...
      <div className="flex overflow-x-auto pb-6 -mx-4 px-4 md:mx-0 md:px-0 gap-4 md:gap-5 snap-x snap-mandatory hide-scrollbar">
        {listings.map((listing) => (
          <div key={listing.id} className="min-w-[140px] md:min-w-[180px] w-[140px] md:w-[180px] flex-none snap-start">
      ...
      {viewMoreHref && (
        <div className="mt-6 sm:hidden">
          <Link href={viewMoreHref} className="flex items-center justify-center w-full py-3 bg-card border border-border text-foreground rounded-[14px] text-[17px] font-normal transition-colors hover:bg-secondary">
            View More
          </Link>
        </div>
      )}
        ...
        <div className="p-3 flex flex-col gap-1 bg-card flex-1">
          ...
          {listing.seller?.university && (
            <div className="mt-auto pt-2 text-[10px] md:text-[11px] font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5 border-t border-border/50">
              <span className="truncate flex items-center gap-1.5">🎓 {listing.seller.university}</span>
            </div>
          )}
        </div>
      </motion.div>
