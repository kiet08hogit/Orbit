...
export default function ProfilePage() {
 ...
 >
 {/* Avatar */}
 <div className={`shrink-0 h-24 w-24 md:h-28 md:w-28 rounded-full border-4 border-zinc-50 shadow-sm overflow-hidden bg-secondary relative ${isEditing ? 'cursor-pointer group' : ''}`}
 ...
 <Input name="name" value={formData.name} onChange={handleChange} className="rounded-xl font-medium bg-secondary border-border focus-visible:ring-black focus-visible:ring-1 h-9 text-sm" />
 </div>
 <div className="space-y-1">
 <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Username</label>
 <Input name="username" value={formData.username} onChange={handleChange} className="rounded-xl font-medium bg-secondary border-border focus-visible:ring-black focus-visible:ring-1 h-9 text-sm" />
 </div>
 <div className="space-y-1">
 <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Major</label>
 ...
 <Textarea name="bio" value={formData.bio} onChange={handleChange} className="rounded-xl font-medium resize-none bg-secondary border-border focus-visible:ring-black focus-visible:ring-1 text-sm" rows={2} />
 </div>
 </div>
 ) : (
 <>
 ...
 <h1 className="text-2xl font-black tracking-tight text-foreground">{userProfile.name || 'Anonymous User'}</h1>
 <div className="flex items-center justify-center h-4 w-4 rounded-full bg-blue-500 text-primary-foreground shadow-sm" title="Verified UIC Student">
 <Shield className="h-2.5 w-2.5" />
 </div>
 </div>
 <p className="text-muted-foreground text-[13px] font-bold mb-3">@{userProfile.username || userProfile.id.slice(0,8)}</p>
 ...
 <Button className="rounded-full h-8 font-bold text-[13px] bg-primary hover:opacity-90 text-primary-foreground shadow-sm transition-all"
 ...
 <>
 <Button className="rounded-full h-8 font-bold text-[13px] bg-primary hover:opacity-90 text-primary-foreground shadow-sm transition-all flex gap-1.5">
 <MessageSquare className="h-3.5 w-3.5" />
 Message
 </Button>
 <Button variant="ghost" className="rounded-full h-8 font-bold text-[13px] text-muted-foreground hover:text-red-600 hover:bg-red-50 transition-all flex gap-1.5">
 ...
 <Button className="rounded-full h-8 font-bold text-[13px] bg-primary hover:opacity-90 text-primary-foreground shadow-sm transition-all min-w-[100px]"
 ...
 {currentUserId === userProfile.clerkUserId ? (
 <Button className="w-full rounded-full h-9 font-bold text-[13px] bg-primary hover:opacity-90 text-primary-foreground shadow-sm"
 onClick={() => setIsEditing(true)}
 >
 Edit Profile
 </Button>
 ...
 <>
 <Button className="w-full rounded-full h-9 font-bold text-[13px] bg-primary hover:opacity-90 text-primary-foreground shadow-sm flex gap-1.5">
 <MessageSquare className="h-3.5 w-3.5" />
 Message Seller
 </Button>
 <Button variant="ghost" className="w-full rounded-full h-9 font-bold text-[13px] text-muted-foreground hover:text-red-600 hover:bg-red-50 flex gap-1.5">
 <AlertTriangle className="h-3.5 w-3.5" />
 Report
 </Button>
 </>
 ...
 <span className="text-[12px] font-bold text-muted-foreground flex items-center gap-1.5"><Star className="h-3.5 w-3.5" /> Rating</span>
 <span className="text-lg font-black text-foreground">New</span>
 </div>
 <div className="flex flex-col items-center md:items-start gap-0.5">
 <span className="text-[12px] font-bold text-muted-foreground flex items-center gap-1.5"><Tag className="h-3.5 w-3.5" /> Active</span>
 <span className="text-lg font-black text-foreground">{listingsCount}</span>
 </div>
 <div className="flex flex-col items-center md:items-start gap-0.5">
 <span className="text-[12px] font-bold text-muted-foreground flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5" /> Swaps</span>
 ...
 <h4 className="font-bold text-[12px] text-muted-foreground">Seller Payouts</h4>
 </div>
 {isStripeLinked ? (
 ...
 <Button onClick={handleConnectStripe} disabled={isLoadingStripe}
 variant="outline"
 ...
 <h2 className="text-lg font-black tracking-tight text-foreground px-2">Listings by {userProfile.name?.split(' ')[0] || 'User'}</h2>
 ...
 <img src={getImageUrl(listing.images[0].url)} alt={listing.title} className="absolute inset-0 w-full h-full object-cover z-0 group-hover:scale-105 transition-transform duration-500" />
 ) : (
 ...
 <div onClick={(e) => { e.preventDefault(); e.stopPropagation(); router.push(`/listings/${listing.id}/edit`); }}
 className="cursor-pointer"
 >
 <Button size="icon" variant="secondary" className="h-8 w-8 rounded-full bg-card text-foreground shadow-sm hover:bg-secondary">
 <Pencil className="h-4 w-4" />
 </Button>
 </div>
 <div onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDeleteListing(listing.id);
 }}
 className="cursor-pointer"
 >
 ...
 <h3 className="text-lg font-black text-foreground mb-1">No active listings</h3>
 <p className="text-muted-foreground text-sm font-medium">This student doesn't have any items for sale right now.</p>
 </div>
 )}
 </div>
 </div>
