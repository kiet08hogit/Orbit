...
export default function SwipePage() {
  ...
  return (
    <div className="relative flex min-h-[calc(100vh-4rem)] flex-col items-center justify-center overflow-hidden bg-[#f5f5f7] font-sans">
      
      {/* Title / Header */}
      <div className="absolute top-8 text-center z-20">
        <h1 className="text-2xl font-black text-zinc-900 tracking-tight flex items-center justify-center gap-2">
          ...
          {activeListing ? (
            <>
              ...
              {nextListing && (
                <motion.div
                  key={nextListing.id}
                  className="absolute inset-0 z-0 flex flex-col overflow-hidden rounded-[24px] bg-white shadow-sm border border-[#e0e0e0]"
                ...
                >
                  ...
                  <div className="p-6 bg-white flex flex-col justify-end h-[40%]">
                    <div className="flex justify-between items-start mb-2">
                      <h2 className="text-2xl font-black text-zinc-900 truncate pr-2">{nextListing.title}</h2>
              ...
              >
                <Backlight blur={40} className="w-full h-full">
                  <div className="relative flex h-full w-full flex-col overflow-hidden rounded-[24px] bg-white shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-[#e0e0e0]">
                    {/* LIKE STAMP */}
                    <motion.div 
                      style={{ opacity: likeOpacity }}
                      className="absolute top-12 left-8 z-30 pointer-events-none"
                    >
                      <div className="border-4 border-emerald-500 text-emerald-500 font-black text-4xl py-1 px-4 rounded-2xl -rotate-12 uppercase tracking-widest bg-white/10 backdrop-blur-sm">
                        LIKE
                      </div>
                    </motion.div>
                    ...
                    >
                      <div className="border-4 border-rose-500 text-rose-500 font-black text-4xl py-1 px-4 rounded-2xl rotate-12 uppercase tracking-widest bg-white/10 backdrop-blur-sm">
                        NOPE
                      </div>
                    </motion.div>
                    ...
                    <div className="relative flex-1 bg-zinc-50 overflow-hidden">
                      ...
                      <div className="absolute top-4 left-4 z-20">
                        <Badge variant="outline" className="bg-white/90 backdrop-blur-md text-zinc-900 border-transparent font-bold px-3 py-1 text-xs shadow-sm uppercase tracking-wider">
                    ...
                    {/* Card Details */}
                    <div className="p-6 bg-white relative z-20 border-t border-zinc-200">
                      <div className="flex justify-between items-start mb-3 gap-2">
                        <h2 className="text-2xl font-black text-zinc-900 leading-tight">{activeListing.title}</h2>
                        <span className="text-2xl font-black text-[#3252DF] shrink-0">${activeListing.price}</span>
                      </div>
                      
                      <p className="text-sm font-medium text-zinc-500 line-clamp-3 mb-5 leading-relaxed">
                      ...
                      <div className="flex items-center gap-3 pt-4 border-t border-zinc-200">
                        <Avatar className="h-10 w-10 border border-zinc-200 shadow-sm">
                          ...
                          <AvatarFallback className="bg-zinc-50 font-bold text-zinc-500">
                            {activeListing.seller?.name?.[0]?.toUpperCase() || "U"}
                          </AvatarFallback>
                        </Avatar>
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-zinc-900">{activeListing.seller?.name || "Anonymous Seller"}</span>
            ...
            <motion.div 
              ...
              className="flex h-full w-full flex-col items-center justify-center rounded-[24px] border border-[#e0e0e0] bg-white shadow-sm p-8 text-center"
            >
              <div className="rounded-full bg-zinc-50 p-6 mb-5">
                <AlertCircle className="h-12 w-12 text-zinc-500" />
              </div>
              <h3 className="text-2xl font-black text-zinc-900 tracking-tight mb-2">You're all caught up!</h3>
