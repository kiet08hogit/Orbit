...
export default function ListingsGridPage() {
 ...
 <Button variant="outline" className="rounded-full border-border text-muted-foreground hover:text-foreground hover:bg-secondary font-bold">
 Clear Search
 </Button>
 </Link>
 </div>
 ...
 {searchQuery ? (
 <>No items found matching "<span className="font-bold text-foreground">{searchQuery}</span>" in <span className="font-bold text-foreground">{CATEGORIES.find(c => c.id === activeCategory)?.label}</span>.</>
 ) : (
 <>There are currently no items available in <span className="font-bold text-foreground">{CATEGORIES.find(c => c.id === activeCategory)?.label}</span>.</>
 )}
 </p>
 {searchQuery ? (
 <Link href="/listings">
 <Button size="lg" className="mt-8 bg-secondary hover:bg-foreground text-primary-foreground font-bold rounded-full shadow-sm">
 Clear Search
 </Button>
 </Link>
 ) : (
 <Link href="/add-product">
 <Button size="lg" className="mt-8 bg-[#DC2626] hover:bg-[#B91C1C] text-primary-foreground font-bold rounded-full shadow-sm shadow-red-500/20">
