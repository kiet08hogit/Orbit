...
export default function ListingDetailPage() {
  ...
  const handleTalkToSeller = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `http://127.0.0.1:3000/chat/conversation/${listing?.seller.clerkUserId}`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleMeetupRequest = async (building: any) => {
    ...
    try {
      ...
      if (res.status === 200 || res.status === 201) {
        ...
        router.push(
          `/chat?id=${conversation.id}&listingId=${listing?.id}&meetupLocation=${meetupLocation}`,
        );
      }
    } catch (err) {
      console.error("Error starting chat:", err);
  ...
  const handleDirectReservation = async () => {
    ...
    try {
      const token = await getToken();
      await axios.post(
        `http://127.0.0.1:3000/transactions/direct-reservation`,
  ...
  const handleWishlistToggle = async () => {
    ...
    try {
      const token = await getToken();
      await axios.post(
        `http://127.0.0.1:3000/listings/${listing.id}/swipe`,
  ...
  const isOwner =
    userId === listing.seller?.clerkUserId || userId === listing.seller?.id;
              ...
              <div className="hidden sm:flex flex-col gap-2 shrink-0">
                {listing.images.map((img, idx) => (
                  <button
                    ...
                    className={`w-[100px] h-[100px] rounded-md overflow-hidden border-2 transition-all duration-200 bg-secondary ${
                ...
                <div className="flex sm:hidden gap-2 mt-3 overflow-x-auto pb-2">
                  {listing.images.map((img, idx) => (
                    <button
                      key={img.id}
                      onClick={() => setActiveImageIndex(idx)}
                      className={`shrink-0 w-16 h-16 rounded-md overflow-hidden border-2 transition-all bg-secondary ${
                        activeImageIndex === idx
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Button
                      ...
                      className={`font-bold h-11 rounded-lg border-border text-sm transition-all ${
                        isWishlisted
        ...
        <DialogContent className="sm:max-w-md rounded-2xl p-6 shadow-2xl border-0 overflow-hidden bg-card">
          ...
          {reportSubmitted ? (
            <div className="py-8 flex flex-col items-center justify-center text-emerald-600">
              ...
              <p className="text-sm text-emerald-600/80 mt-1 text-center">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                <Button
                  ...
                  className="flex-1 rounded-full font-bold h-11 bg-red-600 hover:bg-red-700 text-primary-foreground border-0 shadow-sm"
                >
                  {isSubmittingReport ? (
        ...
        <DialogContent className="sm:max-w-md rounded-2xl p-6 shadow-2xl border-0 overflow-hidden bg-card">
          ...
          <div className="flex flex-col gap-4 mt-4">
            {listing.acceptsProtectedPayment && (
              <div
                className="border-2 border-emerald-500 bg-emerald-50 dark:bg-emerald-500/10 rounded-2xl p-4 cursor-pointer hover:bg-emerald-100 dark:hover:bg-emerald-500/20 transition-colors"
              ...
              >
                ...
                <p className="text-sm text-muted-foreground ml-8">
                  Pay securely with card. Funds are held until you confirm the
                  meetup.
                </p>
              </div>
            )}
            ...
            {listing.acceptsDirectPayment && (
              <div
                className="border-2 border-border hover:border-[#3252DF] hover:bg-[#3252DF]/5 rounded-2xl p-4 cursor-pointer transition-colors"
              ...
              >
                ...
                <p className="text-sm text-muted-foreground ml-8">
                  Pay with Cash, Zelle, or Venmo when you meet the seller.
                </p>
              </div>
            )}
          </div>
