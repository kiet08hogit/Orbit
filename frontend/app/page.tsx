...
export default function Home() {
  return (
    <div className="min-h-screen flex flex-col bg-background relative overflow-x-hidden">
      ...
      <div className="relative flex flex-col lg:flex-row min-h-[700px] w-full items-center justify-between bg-white text-zinc-900 z-10 px-8 lg:px-24 py-24 border-t border-zinc-200">

        {/* Left Side: Text */}
        <div className="flex-1 flex flex-col items-start justify-center text-left z-20 space-y-6 lg:pr-12">
          <h2 className="text-[40px] md:text-[56px] font-semibold tracking-tighter text-black leading-[1.07]">
            What is Orbit?
          </h2>
          <p className="text-[21px] md:text-[28px] text-zinc-600 font-medium leading-[1.14] tracking-tight max-w-xl">
          ...
          <Link href="/about" className="mt-4">
            <button className="inline-flex items-center justify-center bg-black text-white px-[22px] py-[11px] rounded-md text-[16px] font-medium hover:opacity-90 active:scale-95 transition-all cursor-pointer">
              Learn more
            </button>
          </Link>
        </div>
        ...
        <div className="flex-1 relative flex h-[450px] md:h-[550px] w-full items-center justify-center mt-16 lg:mt-0">
          ...
          <div className="absolute top-0 text-center z-10 w-full">
            <div className="text-[34px] font-semibold tracking-tighter text-black">
              Find your items
            </div>
            <div className="text-[17px] text-zinc-500 mt-1">Dorm • Clothes • School • Events • Uni Community</div>
          </div>

          {/* Orbiting Circles */}
          <OrbitingCircles className="size-[50px] border-none bg-transparent" duration={25} delay={20} radius={100}>
            <TabletSmartphone className="h-10 w-10 text-black" />
          </OrbitingCircles>
          <OrbitingCircles className="size-[50px] border-none bg-transparent" duration={25} delay={10} radius={100}>
            <Book className="h-9 w-9 text-black" />
          </OrbitingCircles>
          <OrbitingCircles className="size-[60px] border-none bg-transparent" radius={180} duration={35} reverse>
            <Shirt className="h-10 w-10 text-black" />
          </OrbitingCircles>
          <OrbitingCircles className="size-[60px] border-none bg-transparent" radius={180} duration={35} delay={17} reverse>
            <Glasses className="h-10 w-10 text-black" />
          </OrbitingCircles>
        </div>
      </div>

      {/* Footer Section */}
      <footer className="w-full bg-white text-zinc-900 pt-20 pb-12 px-6 lg:px-12 relative z-10 border-t border-zinc-200">
        <div className="w-full max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-8">

          {/* Logo / Brand */}
          <div className="flex flex-col gap-6 md:col-span-1">
            <Link href="/" className="flex items-center -space-x-1 ">
              <Image src="/Orbit_logo_transparent.png" alt="Orbit Logo" width={80} height={80} className="object-contain invert" />
              <span className=" text-xl sm:text-2xl font-black tracking-tighter text-black">Orbit</span>
            </Link>
          ...
          <div className="flex flex-col gap-4">
            <h4 className="text-[14px] font-semibold text-black mb-2 uppercase tracking-wide">Engineers</h4>
            <Link href="https://www.linkedin.com/in/ntkiet-ho/" className="text-zinc-500 hover:text-black transition-colors text-[14px] font-normal inline-flex items-center gap-1">
          ...
          {/* Company Links */}
          <div className="flex flex-col gap-4">
            <h4 className="text-[14px] font-semibold text-black mb-2 uppercase tracking-wide">Company</h4>
            <Link href="/about" className="text-zinc-500 hover:text-black transition-colors text-[14px] font-normal">About Us</Link>
            <Link href="#" className="text-zinc-500 hover:text-black transition-colors text-[14px] font-normal">Policy & Terms</Link>
          </div>

          {/* Social Links */}
          <div className="flex flex-col gap-4">
            <h4 className="text-[14px] font-semibold text-black mb-2 uppercase tracking-wide">Socials</h4>
            <Link href="#" className="text-zinc-500 hover:text-black transition-colors text-[14px] font-normal">Instagram</Link>
            <Link href="#" className="text-zinc-500 hover:text-black transition-colors text-[14px] font-normal">LinkedIn</Link>
            <Link href="#" className="text-zinc-500 hover:text-black transition-colors text-[14px] font-normal">Email</Link>
          </div>

        </div>

        {/* Copyright */}
        <div className="w-full max-w-7xl mx-auto mt-20 pt-8 border-t border-zinc-200 text-center md:text-left text-zinc-500 font-normal text-[14px] flex flex-col md:flex-row justify-between items-center gap-4">
