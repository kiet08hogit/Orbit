...
export function CustomUserButton() {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-56 rounded-2xl shadow-sm border-zinc-200 dark:border-zinc-200">
        <div className="py-3 px-3">
          <div className="flex flex-col space-y-1">
            <p className="text-sm font-bold leading-none text-zinc-900 dark:text-zinc-50">
              {user.fullName}
            </p>
            <p className="text-xs leading-none text-zinc-500 dark:text-zinc-500">
              {user.primaryEmailAddress?.emailAddress}
            </p>
          </div>
        </div>
        <DropdownMenuSeparator className="bg-zinc-50 dark:bg-zinc-50" />
        <DropdownMenuItem 
          className="cursor-pointer py-2.5 px-3 focus:bg-zinc-50 dark:focus:bg-zinc-50"
